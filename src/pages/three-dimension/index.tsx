...
import {FreeCamera, Vector3, HemisphericLight, MeshBuilder, Scene, Mesh, ArcRotateCamera, FollowCamera, UniversalCamera, Viewport, DeviceSourceManager, DeviceType, StandardMaterial, Color3} from "@babylonjs/core";
...
const onSceneReady = (scene: Scene) => {
    ...
    if (yard3d.bays instanceof Map){
        ground = MeshBuilder.CreateGround('LY',{width:yard3d.bays.get('LY').dimension.width/ 1000, height:yard3d.bays.get('LY').dimension.height/ 1000})
        ground.position.z = yard3d.bays.get('LY').dimension.height / 1000 / 2
        ground.position.x = yard3d.bays.get('ZY').dimension.width / 1000 / 2
        ground = MeshBuilder.CreateGround('ZY',{width:yard3d.bays.get('ZY').dimension.width/ 1000, height:yard3d.bays.get('ZY').dimension.height/ 1000})
        ground.position.z = yard3d.bays.get('LY').dimension.height /1000 + yard3d.bays.get('ZY').dimension.height / 1000 / 2
        ground.position.x = yard3d.bays.get('ZY').dimension.width / 1000 / 2
        ground = MeshBuilder.CreateGround('PY',{width:yard3d.bays.get('PY').dimension.width/ 1000, height:yard3d.bays.get('PY').dimension.height/ 1000})
        ground.position.z =  yard3d.bays.get('ZY').dimension.height/ 1000 + yard3d.bays.get('LY').dimension.height /1000 +yard3d.bays.get('PY').dimension.height /1000 / 2
        ...
        if (yard3d.bays.get('PY').stockSaddles instanceof Array){
            yard3d.bays.get('PY').stockSaddles.forEach((stockSaddle: StockSaddle) => {
                box = MeshBuilder.CreateBox(stockSaddle.id, {})
                box.position.z = stockSaddle.top/1000 + yard3d.bays.get('LY').dimension.height / 1000 
                box.position.x = stockSaddle.left/1000
                box.position.y = stockSaddle.height /2000
            })
        }
        if (yard3d.bays.get('ZY').stockSaddles instanceof Array){
            yard3d.bays.get('ZY').stockSaddles.forEach((stockSaddle: StockSaddle) => {
                box = MeshBuilder.CreateBox(stockSaddle.id, {})
                box.position.z = stockSaddle.top/1000 + yard3d.bays.get('LY').dimension.height / 1000  + yard3d.bays.get('ZY').dimension.height / 1000 
                box.position.x = stockSaddle.left/1000
                box.position.y = stockSaddle.height /2000
    ...
    // setInterval(angular)
    const contextPlate = createTextPlate(`X : ${camera.position.x} \n Y : ${camera.position.y} \n Z : ${camera.position.z}`)
...
const getAngular = (camera:ArcRotateCamera, focusBox: Mesh) => {
    return angular
} 
...
var goBack = (focusBox:Mesh) => {
    if (focusBox.position.x > 0) {
        focusBox.position.x -= 0.5;
    }
}
var goAhead = (focusBox:Mesh) => {
    if (focusBox.position.x < 250) {
        focusBox.position.x += 0.5;
    }
}
...
const Index = () => {
    ...
    return <div>
        {
            yard3d.bays.size > 0 ? <SceneComponent antialias onSceneReady={onSceneReady} onRender={onRender} id="my-canvas"/> : ''
        }
